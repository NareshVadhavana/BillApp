  ...
  private login = async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      const user = await MongoService.findOne(UserModel, {
        ...
        select: 'username role password phoneNumber',
      });

      if (!user) {
        response.statusCode = HTTP_STATUS_CODES.NOT_FOUND;
        throw new Error(ERROR_MESSAGES.COMMON.NOT_FOUND.replace(':attribute', 'email'));
      }

      const isPasswordMatching = await bcrypt.compare(password, user.password);
      if (!isPasswordMatching) {
        throw new Error(ERROR_MESSAGES.COMMON.INVALID.replace(':attribute', 'password'));
      }

      const JWT_SECRET = process.env.JWT_SECRET || 'JWT_SECRET';
      const tokenData = this.createToken(user, JWT_SECRET, 5);
